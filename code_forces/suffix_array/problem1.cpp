...
void solve(std::string const& s){
	...
	// iteration 0
	{
		...
		cs[p[0]] = 0;
		rep(i, 1, n) cs[p[i]] = cs[p[i-1]] + ll(p[i] != p[i-1]);
	}
	...
	while( (ll{1} << k) < ll(n) ) {
		...
		cs[snd(a[0])] = 0;
		rep(i, 1, n) {
			cs[snd(a[i])] = cs[snd(a[i-1])] + ll(fst(a[i]) != fst(a[i-1]));
		}
