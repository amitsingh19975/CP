...
void radix_sort(vi& p, vi const& c) {
	auto const n = p.size();
	vi new_p(n);
	vi count(n, 0);
	...
	vi pos(n);
	pos[0] = 0;
	rep(i, 1, n) pos[i] = pos[i - 1] + count[i - 1];
...
void solve(string const& s){
...

	{
		...
		rep(i, 1, n) cs[p[i]] = cs[p[i - 1]] + ll(s[p[i]] != s[p[i - 1]]);
	}
	...
	while ((1 << k) < n) {
		...
		rep(i, 1, n) {
			auto const cp = mp(cs[p[i]], cs[(p[i] + idx) % n]);
			auto const pp = mp(cs[p[i - 1]], cs[(p[i - 1] + idx) % n]);

			n_cs[p[i]] = n_cs[p[i - 1]] + ll(cp != pp);
		}
