...
static constexpr std::array digit_as_string = {
    std::string_view("one"),
    std::string_view("two"),
...
constexpr int findr(std::string_view const& source) noexcept {
    if (source.size() < 3) {
        return -1;
    }
    rep(i, 0, digit_as_string.size()) {
        auto const p = digit_as_string[i];
        if (p.size() > source.size()) {
            continue;
        }
        bool found = true;
        rep(j, 0, p.size()) {
...
constexpr int find(std::string_view const& source) noexcept {
    if (source.size() < 3) {
        return -1;
    }
    ...
    rep(i, 0, digit_as_string.size()) {
        ...
        auto const s_size = source.size();
        rep(j, size) {
            // std::cout << p[size - j - 1] << ' ' << source[s_size - j - 1] << ", ";
            if (p[size - j - 1] != source[s_size - j - 1]) {
                found = false;
            }
        }
...
ll solve_part_2(){
    ...
    while(std::getline(std::cin, line)) {
        ...
        repr(i, size) {
            ...
            auto const found = findr(std::string_view(line.data() + i, size - i));
            if (found != -1) {
                num = found;
                break;
            }
        ...
        rep(i, size) {
            ...
            auto const found = find(std::string_view(line.data(), i + 1));
            if (found != -1) {
                num += found * 10;
                break;
            }
